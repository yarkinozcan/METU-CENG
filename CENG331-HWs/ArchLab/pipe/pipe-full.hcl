...
/* Here the only adjustment I made is adding the CMPQ instruction. It is the same instruction that I added in part B. The reason why I implemented COMPQ for part C is the fact that CMPQ does NOT write back the result in another register. Usually when we use SUBQ, even though it compares the values, the substracted value is written back to the register. So using SUBQ in the original y86-64 assembly required me to use extra register to store the original value that we want to compare in order not to lose the original value of that register. So using it made me lose one register and one cycle for each comparison. Again it does NOT work with constants only with registers and since it works with registers it requires a regid (register identification) byte. Also it take the values from the source registers rA and rB. And lastly it does not access to memory and it does not write back the result to another register.
