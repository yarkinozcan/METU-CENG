...
class MultiGraph
{
    ...
    //Helper Functions
    GraphEdge   getEdge(const std::string& vertexFromName,
                        const std::string& vertexToName,const std::string& edgeName);
    
    bool        edgeExists(const std::string& vertexFromName,
                        const std::string& vertexToName,const std::string& edgeName);
    
    bool        vertexExists(const std::string& vertexFromName,
                        const std::string& vertexToName) const;
    
    bool        vertex1Exists(const std::string& vertexName) const;
    
    int         getIndex(const std::string& vertexName) const;
    
    std::vector<GraphVertex> getList() const;
